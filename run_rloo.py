# run_rloo.py
# Full RLOO curriculum training: 3→2→0 shot
...
print("=" * 60)
print("  ICRL — RLOO Full Curriculum Training")
...
def compute_rloo_loss(question, response, adv, n_shot):
    prompt   = build_rollout_prompt(question, demos, n_shot)
    ...
    policy_loss = -(token_lp * adv * mask).sum() / n_tok
    kl_loss     = KL_COEFF * (
...
for n_shot in CURRICULUM:
    ...
    for step in range(STEPS_PER_STAGE):
        ...
        for resp, adv in zip(responses, advantages):
            loss = compute_rloo_loss(
                question, resp, adv.item(), n_shot
            )
            loss.backward()
        ...
        all_logs["rloo"]["reward"].append(avg_reward)
        all_logs["rloo"]["response_length"].append(avg_len)
        all_logs["rloo"]["valid_searches"].append(avg_searches)
        all_logs["rloo"]["stage"].append(n_shot)
        total_steps += 1
...
print("\nSaving checkpoint...")
model.save_pretrained("checkpoints/rloo_final")
tokenizer.save_pretrained("checkpoints/rloo_final")

with open("logs/rloo_logs.json", "w") as f:
    json.dump(all_logs, f, indent=2)

print("Checkpoint → checkpoints/rloo_final/")
print("Logs       → logs/rloo_logs.json")
print()
print("=" * 60)
