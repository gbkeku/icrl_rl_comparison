# run_grpo.py — Full GRPO curriculum training: 3→2→0 shot
...
print("=" * 60)
print("  ICRL — GRPO Full Curriculum Training")
...
def generate_rollout(question, n_shot):
    ...
    del inputs, output; clear()
    return response

def compute_rollout_loss(question, response, adv, n_shot):
    prompt   = build_rollout_prompt(question, demos, n_shot)
    full_seq = prompt + response
    enc      = tokenizer(full_seq, return_tensors="pt",
                         truncation=True, max_length=MAX_LEN).to(DEVICE)
    prompt_len = tokenizer(prompt, return_tensors="pt",
                           truncation=True, max_length=MAX_LEN)["input_ids"].shape[1]
    with torch.amp.autocast("cuda", dtype=torch.bfloat16):
        out    = model(**enc)
        logits = out.logits
    log_probs = torch.nn.functional.log_softmax(logits[:, :-1, :], dim=-1)
    ids       = enc["input_ids"][:, 1:]
    token_lp  = log_probs.gather(2, ids.unsqueeze(-1)).squeeze(-1)[0]
    with torch.no_grad():
        with torch.amp.autocast("cuda", dtype=torch.bfloat16):
    ...
    loss  = (-(token_lp * adv * mask).sum() / n_tok +
             KL_COEFF * ((token_lp - ref_lp) * mask).sum() / n_tok)
    loss  = loss / NUM_ROLLOUTS
    del enc, out, logits, log_probs, token_lp
    del ref_out, ref_logits, ref_lp, mask; clear()
    return loss

all_logs    = {"grpo": {"reward": [], "response_length": [], "valid_searches": [], "stage": []}}
...
for n_shot in CURRICULUM:
    ...
    for step in range(STEPS_PER_STAGE):
        ...
        responses = [generate_rollout(question, n_shot) for _ in range(NUM_ROLLOUTS)]
        rewards   = [compute_reward(r, gt_answer) for r in responses]
        advantages = compute_grpo_advantages(rewards)
        optimizer.zero_grad()
        for resp, adv in zip(responses, advantages):
            compute_rollout_loss(question, resp, adv, n_shot).backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
        optimizer.step(); clear()
        avg_reward   = sum(rewards) / len(rewards)
        avg_len      = sum(len(r) for r in responses) / NUM_ROLLOUTS
        avg_searches = sum(r.count("<search>") for r in responses) / NUM_ROLLOUTS
        all_logs["grpo"]["reward"].append(avg_reward)
        all_logs["grpo"]["response_length"].append(avg_len)
        all_logs["grpo"]["valid_searches"].append(avg_searches)
        all_logs["grpo"]["stage"].append(n_shot)
        total_steps += 1
        if step % 10 == 0 or step == STEPS_PER_STAGE - 1:
            preds = [extract_answer(r) for r in responses]
            print(f"  Step {step:3d}/{STEPS_PER_STAGE} | Reward: {avg_reward:.3f} | "
                  f"Searches: {avg_searches:.1f} | Preds: {preds}")
    print(f"  Stage {stage_names[n_shot]} complete!")

print("\nSaving checkpoint...")
model.save_pretrained("checkpoints/grpo_final")
tokenizer.save_pretrained("checkpoints/grpo_final")
with open("logs/grpo_logs.json", "w") as f:
    json.dump(all_logs, f, indent=2)
print("Checkpoint saved -> checkpoints/grpo_final/")
print("Logs saved       -> logs/grpo_logs.json")
print(f"\n{'='*60}")
